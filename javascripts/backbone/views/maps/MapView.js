...
function create_marker(MapPos, MapTitle, MapDesc, InfoOpenDefault, DragAble, Removable, model)
{
  ...
  // Try HTML5 geolocation
    ...
    displayNewMapView: function(model){
    var self = this;
    $('.main').empty();
    var canvas = $(".main");
    canvas.html( this.singleMapTemplate( model ) );
    var lat = parseFloat(model.map_lat);
    var lng = parseFloat(model.map_long);
    var latlng = new google.maps.LatLng(lat, lng);
    var pins = model.pins;
    var model = model;
...
//     console.log("these are the model's pins where I want to call them", model.pins);
